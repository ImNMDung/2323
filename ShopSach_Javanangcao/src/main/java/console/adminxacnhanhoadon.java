package console;

import java.io.IOException;
import java.util.ArrayList;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import bean.dangnhapadminbean;
import bean.lichsubean;
import bo.lichsubo;
import bo.lichsuboadminbo;

/**
 * Servlet implementation class adminxacnhanhoadon
 */
@WebServlet("/adminxacnhanhoadon")
public class adminxacnhanhoadon extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public adminxacnhanhoadon() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
request.setCharacterEncoding("utf-8");
		
		response.setCharacterEncoding("utf-8");
		
		HttpSession session = request.getSession();
		dangnhapadminbean user = (dangnhapadminbean) session.getAttribute("kh"); 
		if ( user == null) {
			response.sendRedirect("dangnhapadmincsl");
		} else {	
			
			
		
		lichsuboadminbo lbo = new lichsuboadminbo();
		ArrayList<lichsubean> dsls = lbo.getlichsu();
		if ( dsls != null) {
			request.setAttribute("dsls", dsls);
		}
		
		RequestDispatcher rd= request.getRequestDispatcher("admin_Xacnhanhoadon.jsp");
		System.out.println("done _ 1");
		rd.forward(request, response);
	}
	}


	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
	}

}
